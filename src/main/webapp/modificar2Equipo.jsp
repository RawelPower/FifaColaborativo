	...
	<div id="page-wrapper">
		...
		<div class="container">
			<div id="content">
				...
				Equipo dataEquipo = connectionBD.getEquipo(Integer.parseInt(cod_equipo));
				%>
				<article> <section class="col-6 col-12-narrower">
				<h3>Modificar Equipo</h3>
				<form action="operaciones.jsp?tipo=modificarEquipo" method="post">
					<div class="row gtr-50">
						<div class="">
							<input type="text" name="nombre" id="nombre"
								value="<%=dataEquipo.getNombre() %>" maxlength="20" />
						</div>
						<div>
							<center>
								<select name="cod_liga" id="cod_liga" value="cod_liga" style="padding: 18px">
									<%
									for (int i = 0; i < listAllLiga.size(); i++){
									%>
									<option value="<%=listAllLiga.get(i).getCod_liga()%>"><%=listAllLiga.get(i).getNombre()%></option>
									<%} %>
								</select>
							</center>
						</div>
						...
						<input type="hidden" name="cod_equipo" id="cod_equipo"
								value="<%=dataEquipo.getCod_equipo() %>" maxlength="20" />
						</div>
					</div>
