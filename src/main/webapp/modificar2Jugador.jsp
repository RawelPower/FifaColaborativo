	...
	<div id="page-wrapper">
		...
		<div class="container">
			<div id="content">
				...
				ArrayList<Equipo> listAllEquipo = connectionBD.listEquipo();
				String cod_jugador = request.getParameter("cod_jugador");
				Jugador dataJugador = connectionBD.getJugadorbycodeJugador(Integer.parseInt(cod_jugador));
				%>
					...
					<div class="row gtr-50">
						<div class="">
							<input type="text" name="nombre" id="nombre"
								value="<%=dataJugador.getNombre() %>" style="width: 450px"
								; maxlength="20" />
						</div>
						<div class="">
							<input type="text" name="pierna" id="pierna"
								value="<%=dataJugador.getPierna() %>" maxlength="9" />
						</div>
						<div>
						<center>
							<select name="cod_equipo" id="cod_equipo" value="cod_equipo"
								style="padding: 18px">
								<%
								for (int i = 0; i < listAllEquipo.size(); i++) {
								%>
								<option value="<%=listAllEquipo.get(i).getCod_equipo()%>"><%=listAllEquipo.get(i).getNombre()%></option>
								<%
								}
								%>
							</select>
						...
						<div class="">
							<input type="text" name="pais" id="pais"
								value="<%=dataJugador.getPais() %>" maxlength="20" />
						</div>
						<div class="">
							<input type="number" name="altura" id="altura"
								value="<%=dataJugador.getAltura() %>" maxlength="4" />
						</div>
						<div class="">
							<input type="hidden" name="cod_jugador" id="cod_jugador"
								value="<%=dataJugador.getCod_jugador() %>" maxlength="4" />
						</div>
